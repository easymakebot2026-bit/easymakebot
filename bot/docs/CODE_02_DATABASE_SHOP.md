...
### `_load_fernet() -> MultiFernet`
- **چه‌کار می‌کند:** متغیر محیطی `ENCRYPTION_KEY` را می‌خواند. این متغیر می‌تواند یک کلید تنها باشد (فرمت قدیمی، هنوز کاملاً پشتیبانی می‌شود) یا چند کلید جدا‌شده با کاما، برای چرخش تدریجی کلید. از روی این لیست یک `cryptography.fernet.MultiFernet` ساخته می‌شود.
...
- در خط پایانی، `_fernet = _load_fernet()` یک نمونه‌ی سراسری (module-level singleton) از `MultiFernet` ساخته می‌شود که هر دو کلاس زیر از آن استفاده می‌کنند.
- **چرخش کلید:** برای عوض کردن کلید بدون از دست دادن دسترسی به داده‌های قدیمی، کلید جدید را **اول** لیست بگذارید و کلید قدیمی را بعدش، جدا با کاما (مثلاً `ENCRYPTION_KEY=new_key,old_key`). نوشتن‌های جدید همیشه با کلید اول رمزنگاری می‌شوند؛ خواندن هر کلید لیست را به‌ترتیب امتحان می‌کند، پس رکوردهای رمزشده با کلید قدیمی هم‌چنان قابل خواندن‌اند. بعد از این‌که همه‌ی مقادیر حساس دوباره ذخیره شدند (مثلاً با وارد کردن دوباره‌ی توکن هر ربات)، کلید قدیمی را می‌توان از لیست حذف کرد.
...
- **موارد خاص/edge case:** اگر `ENCRYPTION_KEY` بدون نگه‌داشتن کلید قدیمی در لیست عوض شود، هر رکورد رمزنگاری‌شده با کلید قبلی دیگر قابل رمزگشایی نیست (`MultiFernet.decrypt` استثنا پرتاب می‌کند). برای چرخش امن، کلید قدیمی را کنار کلید جدید (بعد از کاما) نگه دارید تا همه‌ی داده‌ها با کلید جدید دوباره ذخیره شوند — به بخش «چرخش کلید» بالا مراجعه کنید.
...
### `class RedeemedActivationCode`
جدول `redeemed_activation_codes` — لجر (ledger) سمت بات برای جلوگیری از اعمال دوباره‌ی یک کد فعال‌سازی سایت (`bot/website_client.py`, `bot/handlers/live.py`). چون تماس redeem از مرز آلمان-ایران عبور می‌کند و ممکن است پاسخش گم شود، `website_client.redeem_activation_code` می‌تواند برای یک کد از قبل مصرف‌شده هم دوباره موفقیت گزارش کند (رجوع به توضیح idempotency در `CODE_04`) — این جدول تضمین می‌کند که با وجود چند بار «موفقیت»، `live_until` فقط یک‌بار برای هر کد افزایش پیدا کند.
- `code` (`String(24)`, **primary key**): شکل نرمال‌شده‌ی کد (`EMB-XXXX-XXXX`) — همین یکتا بودنش مکانیزم اصلی جلوگیری از اعمال دوباره است (درج دوم با `IntegrityError` شکست می‌خورد).
- `bot_id` (FK به `built_bots.id`): کدام ربات این کد را مصرف کرده.
- `applied_at`.
