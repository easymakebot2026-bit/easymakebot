...
### `normalize_code(code: str) -> str | None`
- **چه‌کار می‌کند:** دقیقاً معادل پایتونی `emb_actcodes_normalize()` در سمت وردپرس — حروف بزرگ، حذف هر چیزی جز حروف/عدد، حذف پیشوند `EMB`، و اگر طول نهایی ۸ نباشد `None` برمی‌گرداند؛ در غیر این صورت به شکل `EMB-XXXX-XXXX` برمی‌گرداند.
- **چرا لازم است:** هم قبل از فرستادن کد به وب‌سایت استفاده می‌شود (تا فرمت‌های مختلف تایپ کاربر یکسان بشن)، هم به‌عنوان کلید idempotency-ledger سمت بات (`RedeemedActivationCode`، در `bot/handlers/live.py`) — تا کدی که یک‌بار با حروف بزرگ و بار دیگر کوچک تایپ شده، هنوز به‌عنوان همون کد شناخته بشه.

### `redeem_activation_code(code, bot_id, telegram_id, *, username=None, first_name=None, phone=None) -> dict`
- **چه‌کار می‌کند:** ابتدا `code` را با `normalize_code` نرمال می‌کند (اگر فرمتش نامعتبر بود، بدون تماس شبکه‌ای `{"ok": False, "error": "malformed_code"}` برمی‌گرداند)، سپس `POST /wp-json/emb/v1/redeem` می‌زند. بدنهٔ (payload) درخواست را می‌سازد: `code` نرمال‌شده، `bot_id` (به رشته تبدیل‌شده)، `telegram_id` (به عدد صحیح تبدیل‌شده)، و به‌صورت اختیاری `telegram_username`، `telegram_first_name`، و `phone` — فقط اگر مقدار داشته باشند (else کلید اصلاً در payload نیست).
...
- **idempotency روی خطای `already_used` (رفع باگ):** قبلاً اگر پاسخ POST به دلیل قطعی مرز آلمان-ایران گم می‌شد ولی وب‌سایت واقعاً کد را مصرف کرده بود، تلاش بعدی (retry داخلی همین تابع، یا کاربر که کد را دوباره می‌فرستد) با `already_used` مواجه می‌شد و به کاربر «فعال‌سازی ناموفق» نشان داده می‌شد — درحالی‌که پولش را داده و کدش هم سوخته بود ولی رباتش هرگز لایو نمی‌شد. حالا وقتی نتیجهٔ POST دقیقاً `already_used` باشد، تابع یک `_check_code(code)` (فراخوانی `GET /wp-json/emb/v1/check` که هیچ کدی مصرف نمی‌کند) می‌زند؛ اگر آن پاسخ نشان دهد کد واقعاً توسط **همین** `bot_id` و **همین** `telegram_id` مصرف شده (نه شخص/رباتِ دیگری)، این تابع آن را به‌عنوان موفقیت (`{"ok": True, ...}`) برمی‌گرداند، نه شکست.
- **چرا این خودش باعث سوءاستفاده نمی‌شود:** چون این بازیابی به یک کد واقعاً مصرف‌شدهٔ **توسط همین کاربر/بات** محدود است (نه هر کسی)، عملاً معادل «بگو دوباره چی شد» است، نه «کد را دوباره مصرف کن». اما چون همین تابع می‌تواند برای یک کد از قبل با موفقیتِ کامل اعمال‌شده، دوباره `ok: True` برگرداند (مثلاً اگر کاربر دستی دوباره همون کد را بفرستد)، اعمال واقعی `live_until` باید جای دیگری (نه اینجا) دقیقاً یک‌بار تضمین بشه — این کار را جدول `RedeemedActivationCode` در `bot/handlers/live.py:_apply_activation_code` انجام می‌دهد (پایین‌تر توضیح داده شده).
...
5. **idempotency-ledger (رفع باگ):** قبل از هرگونه تغییر در `live_until`، یک رکورد `RedeemedActivationCode(code=ledger_code, bot_id=built_bot.id)` درج می‌شود (`ledger_code` همان کد نرمال‌شده‌ای است که `redeem_activation_code` در پاسخ برگردانده، نه متن خام تایپ‌شدهٔ کاربر). چون ستون `code` این جدول `primary key` است، اگر این کد قبلاً یک‌بار با موفقیت اعمال شده باشد، `session.commit()` با `IntegrityError` شکست می‌خورد؛ این استثنا گرفته می‌شود، `rollback` می‌شود، و پیام «این کد قبلاً برای این ربات فعال شده بود» نشان داده می‌شود **بدون** این‌که `live_until` دوباره تمدید شود. این جدول لازم است چون `website_client.redeem_activation_code` می‌تواند برای یک کد از قبل کاملاً اعمال‌شده، دوباره هم `ok: True` برگرداند (به‌خاطر بازیابی‌اش از `already_used` که در همان فایل توضیح داده شده) — این جدول تضمین می‌کند که هرچند بار «موفقیت» گزارش شود، اثر واقعی (افزودن روز به `live_until`) دقیقاً یک‌بار اعمال می‌شود.
6. **محاسبهٔ تاریخ جدید** (فقط اگر ledger نشان داد این کد تازه است): `base` یا `built_bot.live_until` است (اگر هنوز در آینده باشد — یعنی بات فعلاً لایو است) یا `now` (اگر بات هرگز لایو نبوده یا قبلاً منقضی شده)؛ `until = base + timedelta(days=days)` — یعنی کد روی زمان باقی‌ماندهٔ فعلی add می‌شود، نه جایگزین آن.
7. `live.set_live_until` و در صورت عدم suspension، `start_built_bot`.
8. پیام تبریک با تعداد ماه (اگر `months` در پاسخ بود) یا روز، و در صورت `base != now` یادآوری «از تاریخ انقضای فعلی تمدید شد»، و در صورت suspend‌بودن هشدار مربوطه.
